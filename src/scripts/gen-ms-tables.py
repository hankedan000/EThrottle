...
# column indices for main portion of CSV
COL_TYPE = 0
COL_NAME = 1
COL_BIT_WIDTH = 2
COL_CONST_VALUE = 3
...
def write_header_file(args, nspace, table_list):
	...
	for idx, row in enumerate(table_list):
		...
		if len(row[TL_COL_TYPE]) > 0:
			forward_declare_str += "\n"
			forward_declare_str += "struct %s;" % row[TL_COL_TYPE]

			if len(field_offset_macros) > 0:
				field_offset_macros += "\n"
			field_offset_macros += "#define FIELD_OFFSET_%s(FIELD_NAME) " % row[TL_COL_ENUM_NAME]
		...
		if row[TL_COL_MEM_TYPE] == 'eFlash':
			...
			global_vars.append({'name':row[TL_COL_VAR_NAME], 'type':row[TL_COL_TYPE]})
			c_row.append("&" + row[TL_COL_VAR_NAME])
		else:
			c_row.append(row[TL_COL_VAR_NAME])
		
		# generate table size entry
		if len(row[TL_COL_SIZE]) > 0:
			c_row.append(row[TL_COL_SIZE])
		else:
			c_row.append("sizeof(%s)" % row[TL_COL_TYPE])

		# generate memory type entry
		c_row.append("MegaCAN::TableType_E::%s" % row[TL_COL_MEM_TYPE])
