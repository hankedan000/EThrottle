...
class RowObject(object):
	def __init__(self, nspace, row):
		...
		self.array_size_str = row[COL_ARRAY_SIZE]
		self.array_size = parse_number(nspace, int, self.array_size_str, allow_constants=True)
	...
	def get_c_type_string(self, **kwargs):
		...
		if self.bit_width:
			...
			if len(self.array_size_str) > 0:
				text += "[%s]" % self.array_size_str
			text += ";"
		return text
...
def get_type_size(type, nspace):
	if isinstance(type, MemberedType):
		return traverse_members(type, nspace, None)
	else:
...
def write_header_file(args, nspace, table_list):
	...
	if out_file_h == None:
		out_file_h = open(os.path.splitext(args.csvfile.name)[0] + ".h", 'w')
	out_file_cpp = args.out_file_cpp
	if out_file_cpp == None:
		out_file_cpp = open(os.path.splitext(args.csvfile.name)[0] + ".cpp", 'w')
	...
	for idx, row in enumerate(table_list):
		...
		if idx == 0:
			...
			table_id_enum_str += ",\n"
		table_id_enum_str += DEFAULT_INDENT + "eTI_%s = %d" % (row[4], idx)
		table_flash_offset_str = "-1"
		# if it has a valid type...
		if len(row[0]) > 0:
			forward_declare_str += "\n"
			forward_declare_str += "struct %s;" % row[0]

			if len(field_offset_macros) > 0:
				field_offset_macros += "\n"
			field_offset_macros += "#define FIELD_OFFSET_%s(FIELD_NAME) " % row[4]

			table_size = traverse_members(nspace.get_type_by_name(row[0]), nspace, None)
			table_size_defns.append(["TABLE_SIZE_%s" % row[4], "%d" % table_size])
			if row[3] == 'eFlash':
				table_flash_offset = flash_offset
				flash_offset += table_size
				table_flash_offset_str = "TABLE_FLASH_OFFSET_%s" % row[4]
				table_flash_offset_defns.append([table_flash_offset_str, "%d" % table_flash_offset])
				field_offset_macros += "(%s + offsetof(%s, FIELD_NAME))" % (table_flash_offset_str, row[0])
			else:
				field_offset_macros += "(offsetof(%s, FIELD_NAME))" % (row[0])
	...
	gen_header_str = ""
	gen_header_str += "//============================================================\n"
	gen_header_str += "// this file was autogenerated by gen-ms-tables.py\n"
	gen_header_str += "// source file: %s\n" % os.path.basename(args.csvfile.name)
	gen_header_str += "// date/time: %s\n" % now.strftime("%Y/%m/%d %H:%M:%S")
	gen_header_str += "//============================================================\n"

	# generate the header file code
	out_file_h.write(gen_header_str)
	out_file_h.write('#pragma once\n')
	out_file_h.write('\n')
	out_file_h.write('#include <EEPROM.h>\n')
	out_file_h.write('#include <MegaCAN_ExtDevice.h>\n')
	out_file_h.write('#include <stdint.h>\n')

	out_file_h.write('\n')
	out_file_h.write(make_padded_table(table_size_defns, row_prefix="#define ", default_delim=" "))
	out_file_h.write('\n')
	out_file_h.write(make_padded_table(table_flash_offset_defns, row_prefix="#define ", default_delim=" "))
	out_file_h.write('\n\n')
	out_file_h.write(forward_declare_str)
	out_file_h.write('\n\n')
	out_file_h.write(field_offset_macros)

	constants_table = []
	for constant in nspace.constants_by_name.values():
		row = []
		row.append(constant.type_name)
		row.append(constant.name)
		row.append(constant.const_value)
		constants_table.append(row)
	out_file_h.write('\n\n')
	out_file_h.write(make_padded_table(constants_table, delim_by_col={0:" ",1:" = "}, row_suffix=";"))
	...
	for idx, type in enumerate(nspace.types_by_name.values()):
		...
		if idx > 0:
			out_file_h.write("\n\n")
		out_file_h.write(c_type_str)
		if isinstance(type, MemberedType) and len(type.row_obj.page_id) > 0:
			type_name = type.row_obj.name
			type_size = get_type_size(type, nspace)
			out_file_h.write("\nstatic_assert(sizeof(%s) == %d);" % (type_name, type_size))
			out_file_h.write("\nstatic_assert(sizeof(%s) <= MEGA_CAN_EXT_MAX_FLASH_TABLE_SIZE);" % (type_name))

	global_vars_str = ""
	for var in global_vars:
		if len(global_vars_str) > 0:
			global_vars_str += "\n"
		global_vars_str += "extern %s %s;" % (var['type'], var['name'])
	out_file_h.write('\n\n')
	out_file_h.write(global_vars_str)

	out_file_h.write('\n\n')
	out_file_h.write(table_id_enum_str)
	...
	c_table_defn_str += "};"
	out_file_h.write('\n\n')
	out_file_h.write(c_table_defn_str)

	# generate the source file code
	out_file_cpp.write(gen_header_str)
	out_file_cpp.write("#include \"%s\"\n" % os.path.basename(out_file_h.name))
...
def write_ini_file(args, nspace, **kwargs):
	...
	ini_out_str = gen_header_str + args.ini_in_file.read()
...
def main():
	...
	parser.add_argument(
		'--out-file-cpp',
		help="generated source file path (default is '<csvfile_name>.cpp')",
		type=argparse.FileType('w'))
